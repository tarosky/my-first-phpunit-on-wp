...
recreate_db() {
	...
	if [[ $1 =~ ^(y|yes)$ ]]
	then
		mysqladmin drop $DB_NAME -f --user="$DB_USER" --password="$DB_PASS"$EXTRA_ARGS
		create_db
...
install_db() {
	...
	# create database
	if [ $(mysql --user="$DB_USER" --password="$DB_PASS"$EXTRA_ARGS --execute="SELECT COUNT(*) FROM information_schema.SCHEMATA WHERE SCHEMA_NAME='$DB_NAME'" -s -N) != 0 ]; then
